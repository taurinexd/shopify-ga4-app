...
- **Server-side fallback per browser privacy-strict** — webhook Shopify `orders/create` → relay → GA4 MP. Recupera il ~5% di buyer su Brave/Comet/ETP Strict che oggi sono not-tracked (vedi §10). Trade-off: niente attribution session/cookie, `client_id` da inferire (`order.note_attributes.ga4_cid` se persistito a checkout, altrimenti hash deterministico da `customer.email`).
